...
import { UpdateDoctorProfileRequest } from '../models/doctor.model';
@Injectable({
  providedIn: 'root'
})
export class DoctorService {
  ...
  updateDoctorProfile(updateDoctorProfileRequest: UpdateDoctorProfileRequest, file?: File): Observable<any> {
    const url = `${this.baseUrl}/update-profile`;
    const formData: FormData = new FormData();

    const formattedDateOfBirth = updateDoctorProfileRequest.dateOfBirth
      ? new Date(updateDoctorProfileRequest.dateOfBirth).toISOString().split('T')[0]
      : '';
    ...
    formData.append('gender', updateDoctorProfileRequest.gender ? 'true' : 'false');
  ...
  getAllPatientsOfDoctor(page: number, size: number, patientName: string = ''): Observable<any> {
